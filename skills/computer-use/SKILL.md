...
On macOS, Finder's **Go to Folder** shortcut is `cmd+shift+g`. Desktop automation also requires Accessibility and Screen Recording permissions; use `hermes computer-use doctor` when capture or input fails rather than guessing which permission is missing.
