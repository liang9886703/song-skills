...
This skill controls the **spoken wording** first, but the user may also expect an actual voice message.
...
1. write the reply in this skill's natural spoken Chinese style
2. send it through TTS / voice output if the platform supports it
3. final response should deliver the media, not explain the pipeline
...
Use spoken style; if it is ambiguous whether they want audio delivery, prefer audio in an already-active voice-chat context, otherwise keep the text short and spoken.

Pitfall from session correction:
- Do **not** only generate an MP3 without first shaping the line with this skill; that feels like a random attachment.
- Do **not** only switch to spoken text when the user clearly expects a voice message; they may complain "为啥没有发语音".
- The expected behavior in voice chat is usually: `voice-chat-mode wording -> TTS -> media response`.
